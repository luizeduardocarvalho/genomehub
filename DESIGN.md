# GenomeHub — Design Document

> Living document — design rationale, architecture, and internals.
> **New here? Start with the [README](README.md).** This file is the deep dive.
...
1. **Local import** — researcher already has the FASTA locally; client chunks and registers
2. **Researcher upload** — lab imports then publishes its own genome to an origin
3. **Public databases** *(planned)* — auto-import from NCBI/Ensembl/PLAZA on launch

```bash
genomehub import --fasta tair10.fa --organism "Arabidopsis thaliana" --assembly TAIR10 \
  --output TAIR10.manifest.json
genomehub publish --manifest TAIR10.manifest.json --server https://origin:8443
genomehub seed --tracker https://tracker:9000        # become a seeder (one command)
```
...
Today, contributing splits across existing commands: `seed` (serve bandwidth as a
long-lived node), `sketch` (compute fingerprints), and `work` (claim + run MEM jobs
from a coordinator). A single resource-bounded `contribute` wrapper — CPU/bandwidth
caps, `--once`, scheduling — is planned:
